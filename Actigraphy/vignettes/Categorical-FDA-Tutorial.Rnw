...
4. Change the continious variable AHI in the covariate file into a categorical
variable:
<<>>=
...
5. Match the data in the activity and covariate datasets and since the covariate
...
9. Plot results from FLM results and F-test if indicated (with the categorical
